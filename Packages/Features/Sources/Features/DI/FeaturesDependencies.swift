...
@MainActor
public struct FeaturesDependencies: @unchecked Sendable  {
    ...
    public static var current: FeaturesDependencies {
        guard let deps = _current else {
            fatalError(
                "FeaturesDependencies not configured. Call FeaturesDependencies.configure() from your App module."
            )
        }
        return deps
    }
